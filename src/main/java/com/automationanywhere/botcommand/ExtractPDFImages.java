...
public class ExtractPDFImages {//Identify the entry point for the action. Returns a Value<String> because the return type is String.
    @Execute
            ...
            for (PDPage page : list) {
                ...
                for (COSName c : pdResources.getXObjectNames()) {
                    ...
                    if (o instanceof org.apache.pdfbox.pdmodel.graphics.image.PDImageXObject) {
                        firstPath = String.format(outputPath + fileNameWithoutExt + "-%05d.%s", ++imagenumber,outputType);
                        ImageIOUtil.writeImage(((org.apache.pdfbox.pdmodel.graphics.image.PDImageXObject)o).getImage(), firstPath, 900);
                        resultList.add(new StringValue(firstPath));
                    }
                }
            }
            document.close();
